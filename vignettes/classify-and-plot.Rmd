...
the fit and the assigned classes along with the p-values for both the subclonal and 
the LOH tests.
...
a larger confidence interval on $n_v$, so that the null hypothesis of heterozygosity 
is more likely to be accepted, rejecting sub-clonality or LOH alternatives.
...
Input data contains mutations on the same "target gene" from different samples with 
different purity, that are aggregated in the same single sample "test" for the classification
tesk.
...
This test shows that VAF alone is not a good predictor for the zygosity of variants,
when not corrected for sample purity. TAPACLOTH classifier using the terzile model
on $\text{VAF}/\pi$ ratios, implements the correction and allows for efficient LOH
detection. 