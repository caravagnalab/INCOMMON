purity_from_fit = function(n_binomials, peaks, purity, eps) {
  ...
  if (n_binomials == 2) {
    ...
    if (!is.na(purity) &
        min(abs(purity_hyp - purity)) <= eps)
      # check if at least one is close to the input one
      out = purity_hyp[which.min(abs(purity_hyp - purity))] # choose the estimate that is closest to input
    if (is.na(out)) {
      cli::cli_alert(
        "Purity could not be estimated reliably. Check results of the fit in the output figure"
      )
    }
  }
  return(out)
